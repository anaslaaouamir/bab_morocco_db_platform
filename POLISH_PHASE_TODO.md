# Polish Phase — TODO Checklist
# Bab Morocco BD Intelligence Platform

> Generated: 2026-06-26 | Cross-referenced against live codebase — every item verified.
...
2. **Execution Flow:** Do not work on items without explicit permission. When the user says "Go fix P1-01", the AI must:
   ...
   - Commit the changes to git (e.g., `git commit -m "fix(polish): implement P1-01 — delete prospect"`).
...
## AREA 1 — CRM / Prospection

- [ ] **P1-01 — Delete a prospect**
...
- [ ] **P1-02 — Edit a prospect's details**
  - **Layer:** Missing Frontend Only
  ...
  - **Frontend gap:** `ProspectDrawer` is read-only. Only Notes (on blur) and Stage (dropdown) are editable. Name, email, contact, address, commission, capacity, OTA presence — none are editable.
...
- [ ] **P1-03 — Veille/Perdu prospects are invisible**
  - **Layer:** Missing Frontend Only
  ...
  - **Fix:** Add a "Perdus" filter chip in `FilterBar` or a separate tab on the Prospection page. Show `stage = perdu` prospects with their last update date and a "Réactiver" button that patches stage back to `negociation` or `prospection`.

- [ ] **P1-04 — No server-side pagination for large pipelines**
  - **Layer:** Missing Frontend Only
  - **Backend:** Full pagination implemented — `GET /prospects` returns `page`, `page_size`, `pages`, `total`.
...
- [ ] **P1-05 — Score breakdown not visible in Table view**
  - **Layer:** Missing Frontend Only
  - **Backend:** All 5 criterion scores are returned in `ProspectResponse`.
  - **Frontend gap:** The 5-criterion breakdown bars only appear inside `ProspectDrawer`. The `ProspectTable` score column shows a number only — no way to see why a score is 72 vs 78 without opening the drawer.
...
- [ ] **P2-01 — No empty state on main Outreach panel**
  - **Layer:** Missing Frontend Only
  - **Backend:** No gap.
  - **Frontend gap:** When a prospect is in `outreach` stage but has no generated emails (manually moved to outreach), the main Outreach page right panel shows nothing useful. The "Générer les variantes J0" empty state exists in `ProspectDrawer`'s `OutreachSection` but not on the main outreach page.
...
- [ ] **P2-02 — No inline email body editing before send**
  - **Layer:** Missing Both
...
- [ ] **P2-03 — No outreach stats on Dashboard**
  ...
  - **Fix:** Add outreach KPI chips on the Dashboard: total prospects in outreach stage, total with at least one email sent. Derivable client-side from `allProspects` — no new API endpoint needed.

---

## AREA 3 — Negotiation

- [ ] **P3-01 — No way to view or reactivate `perdu` prospects**
  - **Layer:** Missing Frontend Only
  ...
  - **Fix:** Add a "Perdus" secondary section or tab on the Negotiation page. Show lost prospects with their last message date and a "Réactiver" button (`PATCH /stage → negociation`). Pair with P1-03.

- [ ] **P3-02 — No inline history in the main negotiation panel**
  - **Layer:** Missing Frontend Only
  - **Backend:** `GET /negotiation/{id}/history` exists and returns full message list.
  - **Frontend gap:** Conversation history is only accessible via the dialog button. The main panel shows no running thread — the user must remember what was said from a separate modal.
  - **Fix:** Add a collapsible "Derniers échanges" section between the SubmitMessagePanel and the analysis card, showing the last 2–3 messages inline (direction-aware alignment: inbound left, outbound right). Reuses the already-loaded `historyCache`.

- [ ] **P3-03 — No follow-up timing hint in WaitingForReplyPanel**
  - **Layer:** Missing Frontend Only
  - **Backend:** No gap. Timing logic exists in `outreach_service.py` but not in negotiation.
  - **Frontend gap:** After sending a negotiation response, the `WaitingForReplyPanel` says "En attente de sa réponse" but gives no guidance on follow-up timing (CLAUDE.md §5 defines a 3–7 day window).
  - **Fix:** Add a contextual note in `WaitingForReplyPanel`: "Si pas de réponse d'ici [date J+5], envisagez une relance via le bouton ci-dessous." Compute date client-side from `responded.sentAt`.

---

## AREA 4 — Contracts

- [ ] **P4-01 — `estimated_annual_value` never passed from frontend**
  - **Layer:** Missing Frontend Only
  ...
  - **Frontend gap:** `ContractGenerateDialog` calls `contractsApi.create({ prospect_id })` with no `estimated_annual_value` — always `null`. The $50k human review gate can never trigger from the UI.
  - **Fix:** Add an optional numeric input field in the Step 0 "Réviser" panel: "Valeur annuelle estimée (USD) — optionnel". Pass it to `create()`. Show `human_review_required` badge immediately if threshold exceeded.

- [ ] **P4-02 — No link from ContractCard back to the prospect**
  - **Layer:** Missing Frontend Only
...
- [ ] **P4-03 — Contracts page may show stale data after Conclure redirect**
  - **Layer:** Missing Frontend Only
  - **Backend:** No gap. Contract is auto-created atomically in `PATCH /stage → closing`.
...
- [ ] **P4-04 — `human_review_reason` text is never displayed**
  - **Layer:** Missing Frontend Only
  - **Backend:** `human_review_reason` is returned in `ContractResponse` and stored in `RawContract`.
  - **Frontend gap:** The amber warning badge on `ContractCard` and in `ContractGenerateDialog` shows that human review is required but never shows *why* (e.g., "Commission 7% est inférieure au plancher absolu de 8%").
...
- [ ] **P5-01 — `perdu` stage not counted; no conversion rate**
  - **Layer:** Missing Frontend Only
  - **Backend:** All data present in `allProspects`.
  - **Frontend gap:** Dashboard funnel only includes `prospection → activation_ota`. Prospects in `perdu` are silently excluded. No stat shows negotiation → conversion vs abandonment rate.
  - **Fix:** Add a "Perdus" counter chip below the funnel showing count + "X% des négociations perdues". Derivable from `allProspects` already loaded.

- [ ] **P5-02 — No "Actions en attente" section**
  - **Layer:** Missing Frontend Only
  - **Backend:** All necessary data already returned by existing endpoints.
  - **Frontend gap:** Dashboard has no action queue. Users must navigate to each page to find what needs attention — pending email validations, negotiations awaiting reply, contracts with human review flags.
...
## AREA 6 — General UI / Global Polish

- [ ] **P6-01 — No global prospect name search**
  - **Layer:** Missing Frontend Only
...
- [ ] **P6-02 — No backend health indicator in the UI**
  - **Layer:** Missing Frontend Only
  ...
  - **Fix:** Add a `useEffect` in `AppShell.tsx` that calls `healthApi.check()` on mount. If it fails, show a persistent MUI `Alert` (severity="error") at the top of the layout: "Backend inaccessible — vérifiez que le serveur est démarré."

- [x] **P6-03 — Notes are not persisted to the backend (silent data loss)**
  - **Layer:** Missing Frontend Only — **CRITICAL BUG**
  - **Backend:** `PUT /prospects/{id}` accepts `notes` in `ProspectUpdate`. Fully implemented.
  - **Frontend gap:** `handleNotesChange` in `prospection/page.tsx:196` updates local React state and shows a snackbar — but **never calls the backend**. Notes are lost on every page refresh.
...
- [x] **P6-04 — `prospectsApi.update()` and `prospectsApi.delete()` missing from API client**
  - **Layer:** Missing Frontend Only — **BLOCKS P1-01, P1-02, P6-03**
...
*Polish Phase v1.0 — 2026-06-26*
*Start with P6-04, then P6-03, then P1-01, then P1-02 — in that order.*
